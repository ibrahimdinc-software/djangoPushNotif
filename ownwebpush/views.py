...
from pushuser.models import PushGroupModel, PushUserModel, UserGroupModel
...
@require_POST
@csrf_exempt
def involve_user_to_group(request):
    try:
        post_data = json.loads(request.body.decode('utf-8'))
        print(post_data)
    except ValueError:
    ...
    if 'user' in post_data and 'group' in post_data:
        print(post_data['user'], post_data['group'])
        user = get_object_or_404(PushUserModel, identifier=post_data['user'])
        print(user)
        group = get_object_or_404(PushGroupModel, name=post_data['group'])
        print(group)
        obj, created = UserGroupModel.objects.get_or_create(pushUser=user, pushGroup=group)
        return HttpResponse(status=201)
